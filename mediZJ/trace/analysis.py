...
class TraceAnalyzer:
    ...
    def get_slow_traces(
        ...
        rows = conn.execute(
            """SELECT trace_id, session_id,
                      first_token_time_ms AS duration_ms,
                      first_token_time_ms, mode, agents_involved, question_summary
               FROM traces
               WHERE first_token_time_ms > ?
               ORDER BY first_token_time_ms DESC
               LIMIT ?""",
            (threshold_ms, limit),
        ).fetchall()
        results = []
    ...
    def get_waterfall(self, trace_id: str) -> Dict[str, Any]:
        ...
        first_token_row = conn.execute(
            "SELECT first_token_time_ms FROM traces WHERE trace_id = ?",
            (trace_id,),
        ).fetchone()
        first_token_time_ms = (
            first_token_row["first_token_time_ms"]
            if first_token_row is not None
            else None
        )
